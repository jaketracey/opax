...
test('MCP tools return usable record citations and bound oversized responses',async()=>{
 ...
 let read=async path=>Response.json(path.startsWith('/api/search-all')?{results:[{slug:'speech-931754',title:'Housing'},{href:'/subject/supplier/example',title:'Supplier'},{href:'/money/grants?jur=qld&open=abn%3A97694995462',title:'Test Recipient'}]}:{slug:'speech-931754',text:'Source text'});
 ...
 const grantRecipient=await send('tools/call',{name:'read_grant_recipient',arguments:{jurisdiction:'qld',id:'abn:97694995462'}});assert.equal(grantRecipient.isError,false);const grantData=JSON.parse(grantRecipient.content[0].text);assert.equal(grantData.opax_url,'https://opax.test/money/grants?jur=qld&open=abn%3A97694995462');assert.equal(grantData.grants.find(g=>g.id==='qld-1').source_url,'https://www.grants.gov.au/Ga/Show/test-guid-123');assert.equal(grantData.grants.find(g=>g.id==='qld-2').source_url,undefined);
