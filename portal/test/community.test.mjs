...
test('MCP tools return usable record citations and bound oversized responses',async()=>{
 ...
 const grantRecipient=await send('tools/call',{name:'read_grant_recipient',arguments:{jurisdiction:'qld',id:'abn:97694995462'}});assert.equal(grantRecipient.isError,false);const grantData=JSON.parse(grantRecipient.content[0].text);assert.equal(grantData.opax_url,'https://opax.test/money/grants/qld/recipient/abn%3A97694995462');assert.equal(grantData.grants.find(g=>g.id==='qld-1').source_url,'https://www.grants.gov.au/Ga/Show/test-guid-123');assert.equal(grantData.grants.find(g=>g.id==='qld-2').source_url,'https://www.data.qld.gov.au/dataset/b102c881-2c7f-484a-a8b6-b056fe318964');assert.equal(grantData.grants.find(g=>g.id==='qld-2').source_kind,'dataset');assert.match(grantData.coverage.value_basis,/expenditure rows, not distinct awards/);
...
test('saved conversations are private, owner controlled, and the newer write wins',async()=>{const f=fixture(),a=await f.login(),b=await f.login('b@example.com');
 try{
  const thread=[{role:'user',text:'What grants has Barnaby Joyce been involved in?'},{role:'answer',text:'None recorded.',sources:[]}];
  assert.equal((await f.call('chats/conv-0001-aaaa','PUT',{title:'Joyce grants',kind:'all',updated:1000,thread})).status,401);
  assert.equal((await f.call('chats/conv-0001-aaaa','PUT',{title:'Joyce grants',kind:'all',updated:1000,thread:[]},a.cookie)).status,400);
  assert.equal((await f.call('chats/conv-0001-aaaa','PUT',{title:'Joyce grants',kind:'all',updated:1000,thread},a.cookie)).status,200);
  const stale=await (await f.call('chats/conv-0001-aaaa','PUT',{title:'older','kind':'all',updated:900,thread},a.cookie)).json();
  assert.equal(stale.stale,true);assert.equal(stale.updated_at,1000);
  const list=await (await f.call('chats','GET',undefined,a.cookie)).json();
  assert.equal(list.chats.length,1);assert.equal(list.chats[0].title,'Joyce grants');assert.equal(list.chats[0].turns,1);assert.equal(list.chats[0].updated_at,1000);
  const one=await (await f.call('chats/conv-0001-aaaa','GET',undefined,a.cookie)).json();
  assert.deepEqual(one.chat.data.thread,thread);
  assert.equal((await f.call('chats/conv-0001-aaaa','GET',undefined,b.cookie)).status,404);
  assert.equal((await f.call('chats/conv-0001-aaaa','PUT',{title:'mine now',kind:'all',updated:5000,thread},b.cookie)).status,404);
  assert.equal((await f.call('chats/conv-0001-aaaa','DELETE',undefined,b.cookie)).status,404);
  assert.equal((await (await f.call('chats','GET',undefined,b.cookie)).json()).chats.length,0);
  assert.equal((await f.call('chats/conv-0001-aaaa','PUT',{title:'Joyce grants, again',kind:'all',updated:2000,thread},a.cookie)).status,200);
  assert.equal((await (await f.call('chats/conv-0001-aaaa','GET',undefined,a.cookie)).json()).chat.title,'Joyce grants, again');
  assert.equal((await f.call('chats/conv-0001-aaaa','DELETE',undefined,a.cookie)).status,200);
  assert.equal((await f.call('chats/conv-0001-aaaa','GET',undefined,a.cookie)).status,404);
 }finally{f.db.close()}
});
