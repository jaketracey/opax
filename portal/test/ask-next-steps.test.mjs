...
test('a funding next step preserves the map selection and financial years',()=>{
 assert.deepEqual({...context.moneyNextSteps(answer('Donor','Woodside Energy'))},{map:expectedMap,donor:'Woodside Energy'});
 assert.deepEqual({...context.moneyNextSteps(answer('Donor → party','Woodside Energy → Liberal'))},{map:expectedMap,donor:'Woodside Energy'});
});
test('a party ranking names no donor, and an answer without the map link offers no map',()=>{
 assert.deepEqual({...context.moneyNextSteps(answer('Recipient party','Liberal'))},{map:expectedMap,donor:''});
...
test('the rendered funding action names its map destination and retains the existing speech follow-up',()=>{
 ...
 const [map,followup]=nav.children;
