...
test('unquoting a misquote shifts every citation after each removed mark, by code point',()=>{
 const answer='😀 She said “this is not in any passage at all, believe me” and then [x]. Then “neither is this second long quotation here” too.';
 const marks=[...answer.matchAll(/[“”]/g)].map(m=>Array.from(answer.slice(0,m.index)).length);
 const c1=marks[0]+3, c2=Array.from(answer).length-1; // one range between the quotes, one at the end
 const payload={answer,citations:{a:[[c1,c1+1]],b:[[c2,c2+1]]},sources:[{resource:'a',snippet:'Unrelated passage text.'},{resource:'b',snippet:'Another unrelated passage.'}]};
 const raw={retrieval_results:{resources:{a:{fields:{'t/body':{paragraphs:{'a/t/body/0-9':{text:'Unrelated passage text.'}}}}},b:{fields:{'t/body':{paragraphs:{'b/t/body/0-9':{text:'Another unrelated passage.'}}}}}}}};
 const out=api.unquoteUnsupported(payload,raw);
 assert.equal(out.answer,'😀 She said this is not in any passage at all, believe me and then [x]. Then neither is this second long quotation here too.');
 assert.deepEqual(plain(out.citations.a),[[c1-1,c1]]);assert.deepEqual(plain(out.citations.b),[[c2-4,c2-3]]);
 assert.equal(Array.from(out.answer)[out.citations.b[0][0]],'.');
 assert.equal(api.unquoteUnsupported({...payload,answer:'No quotes here.'},raw).answer,'No quotes here.');
});
