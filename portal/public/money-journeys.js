import { buildMoneyJourneys } from './money-journeys-data.js?v=journeys-1';
...
export function mountMoneyJourneys(controls, story, stage, data, map, options = {}) {
  ...
  function render() {
    ...
    story.innerHTML = `<div class="journey-story-top"><span>${esc(active.title)}</span><button type="button" data-action="exit" aria-label="End guided journey">×</button></div>
  ...
  function schedule() {
    ...
    timer = setTimeout(() => {
      ...
      reason = ''; render(); present(); schedule(); options.onRoute?.(active.id, step);
    }, STEP_MS);
  }
  function pause(why = '') {
  ...
  function choose(id, requestedStep = 0, announce = true) {
    ...
    render(); present();
    if (announce) options.onRoute?.(active.id, step);
  }
  function exit() {
  ...
  function onAction(event) {
    ...
    switch (button.dataset.action) {
      ...
      case 'play':
        ...
        if (!active || reduced.matches || document.hidden) break;
        if (step === active.steps.length - 1) step = 0;
        playing = true; reason = ''; render(); present(); schedule(); options.onRoute?.(active.id, step); break;
    }
  ...
  return {
    pause,
    setRoute(id, requestedStep = 0) {
      if (destroyed) return;
      if (id === active?.id && Number(requestedStep) === step) return;
    ...
    destroy() {
      ...
      controls.removeEventListener('click', onLens); story.removeEventListener('click', onAction); story.removeEventListener('keydown', onKey);
