...
function multiplePartiesJourney(view, requested) {
  ...
  return {
    ...
    steps: [
      ...
      { title: 'Put the destinations side by side', body: `Compare the ${shown.length} largest connections. Smaller or undisclosed receipts may be missing from the map.`, metric: metric('Receipts on the links shown', amount(shown)), scene: scene(donor.id, shown.map(e => e.target), shown), links: [donorLink(donor)] },
    ],
  };
}
