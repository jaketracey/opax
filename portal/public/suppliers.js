...
function lifecycle(root) {
  ...
  let active = true;
  const cleanups = [];
  return { signal: controller.signal, alive: () => active, cleanup(fn) { cleanups.push(fn); }, destroy() { active = false; controller.abort(); cleanups.forEach((fn) => fn()); } };
}
...
function coverageHTML(meta) {
  const snapshot = meta?.generated_at || meta?.snapshot_at;
  const window = meta?.published_from && meta?.published_to ? ` Notices published ${date(meta.published_from)} to ${date(meta.published_to)}.` : "";
...
function agencyChart(agencies, total) {
  ...
  return `<section class="supplier-section"><h3 class="subject-section-title">Who awards the contracts</h3><p class="supplier-section-note">${agencies.length > 8 ? "The eight largest agencies" : "Agencies"} by recorded contract value.</p><ol class="supplier-bars">${rows.map((row) => `<li><div><span>${esc(row.name)}</span><strong>${currency(row.total)}</strong></div><svg viewBox="0 0 100 5" preserveAspectRatio="none" aria-hidden="true"><rect width="100" height="5" class="supplier-bar-track"/><rect width="${Math.max(0, Number(row.total) / max * 100).toFixed(2)}" height="5" class="supplier-bar-fill"/></svg><small>${number(row.count)} ${Number(row.count) === 1 ? "contract" : "contracts"}${total > 0 ? ` · ${percent(Number(row.total) / total * 100)} of recorded value` : ""}</small></li>`).join("")}</ol></section>`;
}

function yearChart(years, undated) {
...
async function mountFunding(root, links, life) {
  ...
  try {
    ...
    root.innerHTML = `<h3 class="subject-section-title">Also in the funding record</h3><p class="supplier-section-note">Party receipts linked to this supplier’s recorded identity, from the money map’s selected donor data. These totals cover a different set of records and dates from the contracts above.</p>${rows.length ? `<ol class="supplier-bars">${rows.map((row) => `<li><div><a href="/subject/party/${encodeURIComponent(row.name)}">${esc(row.name)}</a><strong>${currency(row.total)}</strong></div><svg viewBox="0 0 100 5" preserveAspectRatio="none" aria-hidden="true"><rect width="100" height="5" class="supplier-bar-track"/><rect width="${(row.total / max * 100).toFixed(2)}" height="5" class="supplier-bar-fill"/></svg><small>${number(row.count)} recorded receipts</small></li>`).join("")}</ol>` : matchedNodes.length ? `<p><strong>${currency(nodeTotal)}</strong> across ${number(nodeCount)} recorded receipts${yearSpan ? ` (${yearSpan})` : ""}. The party breakdown is not available in this map export.</p>` : "<p>This linked donor is not present in the current map export.</p>"}${matchedNodes.length ? '<button type="button" class="supplier-button supplier-map-toggle">Explore the money map</button><div class="supplier-money-map" hidden></div>' : ""}<p class="fineprint">A connection between records does not establish influence over a contract award. ${links.map((link) => `${esc(link.name)}: ${esc(identityMethod(link.method))}.`).join(" ")}</p>`;
