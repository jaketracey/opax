...
export function mountGrantsResearch(root,{focus=false}={}) {
  ...
  root.innerHTML=`<div class="allocation-intro"><h1 tabindex="-1">Where community funding goes</h1><p>See what a grant was for, who it was awarded to and where the project is.</p></div>
    ...
    <section class="allocation-explorer" aria-label="Find community funding">
      <div class="allocation-map-layout" id="allocation-map-layout"><aside class="allocation-detail" id="allocation-detail" aria-label="Selected project"></aside><div class="allocation-map-column"><p id="allocation-map-caption" class="allocation-map-caption"></p><div class="allocation-map-frame"><div id="allocation-map" aria-label="Map of verified project sites"></div><div class="allocation-map-actions" data-ui-size="compact"><button class="ui-button" id="allocation-fit" type="button">Show all sites</button><button class="ui-button" id="allocation-move" type="button" aria-pressed="false">Move map</button></div><div id="allocation-map-empty" class="allocation-map-empty" hidden><h3>No project sites mapped here yet</h3><p>You can still explore the funding records for this view.</p><button class="ui-button" type="button" data-action="list">See matching records</button></div></div><p id="allocation-map-status" class="allocation-map-status" role="status" hidden></p></div></div>
      ...
      <div class="allocation-timeline" id="allocation-timeline"><div><label for="allocation-year">Show grants published by the end of <output id="allocation-year-label" for="allocation-year"></output></label><button type="button" class="text-link" id="allocation-latest">All years</button></div><input id="allocation-year" type="range" step="1" aria-describedby="allocation-time-note allocation-year-summary"><div class="allocation-year-ends" aria-hidden="true"><span id="allocation-first-year"></span><span id="allocation-last-year"></span></div><div class="allocation-year-navigation"><button type="button" class="ui-button" id="allocation-year-previous">Previous year</button><button type="button" class="ui-button" id="allocation-year-next">Next year</button></div><div id="allocation-year-summary" role="status" aria-live="polite" aria-atomic="true"></div><div id="allocation-year-projects"></div><p id="allocation-time-note">These are selected examples. Amounts are the latest recorded award values, including any later updates; they do not show payments made that year.</p></div>

      <div id="allocation-projects"></div><div class="allocation-list-actions"><button class="ui-button" id="allocation-more" hidden>Show more projects</button><button class="text-link" id="allocation-export">Download these records</button></div>
    </section>
