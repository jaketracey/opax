...
function failed(root, retry) {
  ...
  root.innerHTML = '<div class="supplier-page" role="alert"><h2>Agency records unavailable</h2><p>The records could not be loaded.</p><button class="ui-button" type="button">Try again</button></div>';
...
export function mountAgencyDirectory(root, helpers = {}) {
  ...
  async function load() {
    ...
    try {
      ...
      root.removeAttribute('aria-busy');
      root.innerHTML = `<div class="supplier-page agency-directory-page"><div class="subject-head"><h2 id="subject-title" tabindex="-1">Government agencies</h2><p class="subject-tag">Who awards Commonwealth contracts, and which companies receive them.</p></div><div class="ui-toolbar"><label class="ui-field">Find an agency<input class="ui-input" type="search" name="agency-query" placeholder="Agency name" autocomplete="off"></label><label class="ui-field">Sort by<select class="ui-input" name="agency-sort"><option value="total">Contract value</option><option value="count">Number of contracts</option><option value="supplier_count">Number of suppliers</option><option value="name">Name</option></select></label></div><p class="supplier-result-count" role="status"></p><div class="supplier-directory"></div><div class="supplier-more"></div><p class="fineprint">Agency names appear as recorded. Renamed departments are separate entries; no succession or combined history is assumed.</p>${coverageHTML(data.meta)}</div>`;
      ...
      function render() {
        ...
        more.innerHTML = rows.length > visible ? '<button class="ui-button" type="button">Show more agencies</button>' : '';
...
function renderProfile(root, profile, meta, helpers, life) {
  ...
  function suppliers() {
    ...
    const more = root.querySelector('.agency-suppliers-more'); more.innerHTML = rows.length > suppliersVisible ? '<button type="button" class="ui-button">Show more suppliers</button>' : '';
  ...
  function contracts() {
    ...
    const more = root.querySelector('.supplier-contract-more'); more.innerHTML = rows.length > contractsVisible ? '<button type="button" class="ui-button">Show more contracts</button>' : '';
