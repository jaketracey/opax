...
function renderProfile(root, profile, meta, helpers, life) {
  ...
  function suppliers() {
    ...
    const rows = profile.suppliers.filter(s => `${s.name} ${s.abn || ''}`.toLocaleLowerCase().includes(q));
