...
async function mountDiscoveryMap(signal) {
  ...
  try {
    ...
    const { mountMoneyMap } = await import("/money-map.js?v=award-card-1");
    if (!current()) return;
    root.textContent = "";
...
async function mountMoney(jurParam, industry, params = new URLSearchParams()) {
  ...
  const cfg = MONEY_JURISDICTIONS[jur];
  try {
    const [{ mountMoneyMap }, data, journeysModule, researchModule, recordsModule] = await Promise.all([import("/money-map.js?v=award-card-1"), loadMoneyFile(jur), import("/money-journeys.js?v=official-ia-ux-20260908-2"), import("/map-research.js?v=ia-ux-20260908-2"), import("/money-records.js?v=ia-ux-20260908-2")]);
...
async function mountFrontMap() {
  ...
  try {
    const [mod, data] = await Promise.all([import("/money-map.js?v=award-card-1"), loadMoneyData()]);
