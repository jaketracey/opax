...
async function mountDiscoveryMap(signal) {
  ...
  try {
    ...
    const { mountMoneyMap } = await import("/money-map.js?v=mobile-zoom-1");
    if (!current()) return;
    root.textContent = "";
...
async function mountMoney(jurParam, industry, params = new URLSearchParams()) {
  ...
  const cfg = MONEY_JURISDICTIONS[jur];
  try {
    const [{ mountMoneyMap }, data, journeysModule] = await Promise.all([import("/money-map.js?v=mobile-zoom-1"), loadMoneyFile(jur), import("/money-journeys.js?v=story-1")]);
...
async function mountFrontMap() {
  ...
  try {
    const [mod, data] = await Promise.all([import("/money-map.js?v=mobile-zoom-1"), loadMoneyData()]);
