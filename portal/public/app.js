...
async function mountDiscoveryMap(signal) {
  ...
  try {
    ...
    const { mountMoneyMap } = await import("/money-map.js?v=profile-primary-1");
    if (!current()) return;
    root.textContent = "";
...
async function mountFrontMap() {
  ...
  try {
    const [mod, data] = await Promise.all([import("/money-map.js?v=profile-primary-1"), loadMoneyData()]);
