...
export function sourcePath(value:unknown){if(value==null||value==='')return null;const path=text(value,1,600,'Record link');if(!/^\/(?:record|subject|search|money|bills?|reports|connections)(?:[/?#]|$)/.test(path)||/[\\\u0000-\u0020]/.test(path))throw new CommunityError(400,'Use a link to a record or page on Opax.');return path}
