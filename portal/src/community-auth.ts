...
export async function authRoute(req:Request,env:Env,path:string):Promise<Response|null>{
 if(path==='/api/community/auth/request'&&req.method==='POST'){
  ...
  try{await env.COMMUNITY_EMAIL.send({from:{email:env.COMMUNITY_EMAIL_FROM,name:'Opax'},to:email,subject:'Your sign-in link for Opax',text:`Sign in to your Opax community account:\n\n${link}\n\nThis link works once and expires in 15 minutes. If you did not request it, you can ignore this email. Signing in does not start a paid subscription.`,html:`<p>Here is your link to the Opax community.</p><p><a href="${link}">Sign in to Opax</a></p><p>This link works once and expires in 15 minutes. If you did not request it, you can ignore this email. Signing in does not start a paid subscription.</p>`})}catch{await env.COMMUNITY_DB.prepare('DELETE FROM login_links WHERE token_hash=?').bind(hash).run();throw new CommunityError(503,'We could not send your sign-in email. Please try again shortly.')}
