...
const NAMED_SPEECH = /^(?:what|how)\s+(?:has|have|did|does|would|might)\s+(?:(?:Senator|MP|Mr|Mrs|Ms|Dr)\.?\s+)?(.{3,80}?)\s+(?:say|said|speak|spoken|describ(?:e|ed)|argu(?:e|ed)|propos(?:e|ed)|recommend(?:ed)?)\b/i
