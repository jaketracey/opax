...
The verified-proposal fallback is tagged by the server so it can reuse the normal answer cache; generic unverified evidence-only responses remain uncached. This avoids repeatedly paying to regenerate the same failed summary. The final suite contains 374 tests.
