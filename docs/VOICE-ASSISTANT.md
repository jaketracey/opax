...
The bottom-right **Talk to Opax** panel is optional. A signed-in community member receives 600 seconds in total, shared across tabs and devices. Opening the panel does not request microphone access or load the voice SDK. Start talking explicitly begins capture; ending, closing, navigating or leaving the tab stops it. Ask and public records remain available when voice time runs out.
...
1. Apply `portal/migrations/0003_voice.sql` to the target COMMUNITY_DB using Wrangler migrations.
