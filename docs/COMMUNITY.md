...
Preview: `https://staging.opax.com.au/community`. Production activation still requires Cloudflare Email Sending authorization and a real magic-link delivery/sign-in check. Cloudflare OAuth was refreshed and Email Sending is enabled for login.opax.com.au, with public SPF, DKIM and DMARC records verified. Staging accepts real sign-in requests and returns provider message IDs. A direct CLI delivery check reports the recipient queued. Inbox delivery is still being verified. Keep the goal open until production is enabled and verified.
